...
func run() error {
	...
	http.Handle("/", http.HandlerFunc(func(rw http.ResponseWriter, r *http.Request) {
		...
		default:
			...
			isWasmRequest := strings.HasSuffix(r.URL.Path, "${{ steps.info.outputs.REPO_NAME }}_jswasm.wasm")
			if !isWasmRequest {
				r.URL.Path = filepath.Join(r.URL.Path, "${{ steps.info.outputs.REPO_NAME }}_jswasm.wasm")
			}
			...
			if err != nil {
				...
				rw.Write([]byte(
					fmt.Sprintf("cd %v\n", subDir)))
				rw.Write([]byte(`(powershell) $Env:GOOS="js"; $Env:GOARCH="wasm"; go build -o ${{ steps.info.outputs.REPO_NAME }}_jswasm.wasm .`))
				rw.Write([]byte("\n"))
				rw.Write([]byte("(unix)       GOOS=js GOARCH=wasm go build -o ${{ steps.info.outputs.REPO_NAME }}_jswasm.wasm .\n"))
